...
export async function POST(request: Request) {
  if (process.env.NODE_ENV !== 'development') {
    return NextResponse.json({ error: 'Connection testing is available in development only.' }, { headers: noStoreHeaders, status: 404 })
  }
  ...
  if (!apiKey) {
    return NextResponse.json({ configured: false, error: 'DEEPSEEK_API_KEY is not configured.' }, { headers: noStoreHeaders, status: 400 })
  }
  ...
  try {
    ...
    return NextResponse.json({ configured: true, latencyMs: Date.now() - startedAt, ok: true }, { headers: noStoreHeaders })
  } catch (error) {
